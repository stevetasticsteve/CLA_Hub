...
class NewCEPageTest(CETestBaseClass):
    ...
    def test_new_CE_page_saves_single_picture(self):
        try:
            ...
            self.assertTrue(os.path.exists('uploads/CultureEventFiles/3/images'), 'upload folder doesn\'t exist')
            folder_contents = os.listdir('uploads/CultureEventFiles/3/images')
            self.assertIn('test_pic1.jpg', folder_contents, 'Uploaded picture not in upload folder')
            # check smaller than 1Mb
            self.assertTrue(os.path.getsize('uploads/CultureEventFiles/3/images/test_pic1.jpg') < 1000000, 'picture too big')
            ...
            self.assertContains(response, '<img src="/uploads/CultureEventFiles/3/images/test_pic1.jpg')
        finally:
    ...
    def test_new_CE_page_can_save_text_with_audio(self):
        try:
            ...
            self.assertRedirects(response, '/CE/3')
            # Test new CE in .db
            self.assertEqual(len(models.CultureEvent.objects.all()), 3, 'New CE not saved')
    ...
    def test_single_question_submit(self):
        ...
        self.assertEqual(q[1].asked_by, 'Tester')
        self.assertEqual(q[1].last_modified_by, 'Tester')
