...
if CE.settings.auto_cross_reference:
    description_placeholder = (
        "Auto cross reference is on. If you type the (lowercase) title of an existing CE"
        " it will become a hyperlink. \nField is html enabled"
    )
else:
    description_placeholder = (
        "Auto cross reference is off. To insert a cross reference put the title of an existing"
...
class TextForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "audio": forms.ClearableFileInput(
                attrs={"class": "form-control", "accept": ".mp3, .m4a"}
            ),
            "speaker_plain_text": forms.TextInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "You can write a person's ID number here to create a link",
                }
            ),
            "orthographic_text": forms.Textarea(
                attrs={"class": "form-control", "rows": 25}
            ),
        }
        labels = {"speaker_plain_text": "Speaker"}
    ...
    def save(self, **kwargs):
        ...
        try:
            if (
                self.cleaned_data["phonetic_text"]
                or self.cleaned_data["orthographic_text"]
                or self.cleaned_data["audio"]
                or self.cleaned_data["text_title"]
            ):
