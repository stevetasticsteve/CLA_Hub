...
text_form_factory = forms.inlineformset_factory(
    CE.models.CultureEvent, CE.models.Text, form=CE.forms.TextForm, extra=0
)

question_form_factory = forms.inlineformset_factory(
    CE.models.CultureEvent, CE.models.Question, form=CE.forms.QuestionForm, extra=0
)

visits_form_factory = forms.inlineformset_factory(
    CE.models.CultureEvent, CE.models.Visit, form=CE.forms.VisitsForm, extra=0
)
...
def home_page(request):
    ...
    context = {"CEs": CEs, "paginator": CEs, "title": "CE home", "search_context": "CE"}
...
def alphabetical(request):
    ...
    ces = paginator.get_page(request.GET.get("page"))
    context = {"CEs": ces, "paginator": ces, "title": "CEs alphabetically"}
...
def view(request, pk):
    ...
    ce.description = markdown.markdown(
        ce.description
    )  # don't need to clean it since model handles that
...
def edit(request, pk):
    ...
    if request.method == "POST":
        ...
        text_form = text_form_factory(
            request.POST, request.FILES, prefix="text", instance=ce
        )
        question_form = question_form_factory(
            request.POST, request.FILES, prefix="question", instance=ce
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                question_form = question_form_factory(
                    request.POST, prefix="question", instance=ce
                )
                visit_form = visits_form_factory(
                    request.POST, prefix="visit", instance=ce
                )
...
def questions_chron(request):
    ...
    context = {"Questions": questions, "paginator": questions, "CEs": set_ces}
...
def questions_alph(request):
    ...
    context = {"Questions": q, "paginator": q, "CEs": set_ces, "title": "Questions"}
...
def questions_recent(request):
    questions = (
        Question.objects.all().exclude(answer="").order_by("ce", "-last_modified")
    )
    paginator = Paginator(questions, 25)
    questions = paginator.get_page(request.GET.get("page"))
    set_ces = set([i.ce for i in questions])
...
def OCM_home(request):
    ...
    tagged_OCM = {
        tag.name: tag.num for tag in tags if tag.name in OCM_categories.categories
    }
...
def tag_detail_page(request, slug):
    ...
    context = {"tag": tag, "paginator": CEs, "CEs": CEs, "title": tag}
...
def tag_list_page(request):
    ...
    tags = (
        Tag.objects.all().annotate(num=Count("taggit_taggeditem_items")).order_by("-id")
    )
...
def search_CE(request):
    template = "search_results.html"
    search = request.GET.get("search")
    results = CultureEvent.objects.filter(Q(title__icontains=search)).order_by(
        "-last_modified"
    )
    paginator = Paginator(results, 25)
    results = paginator.get_page(request.GET.get("page"))
...
def text_search(request):
    template = "search_results.html"
    search = request.GET.get("search")
    results = Text.objects.filter(Q(text_title__icontains=search)).order_by(
        "-last_modified"
    )
    for t in results:
        t.orthographic_text = CE.utilities.format_text_html(t)
